...
test('wrapping mso on div with max and full widths', async() => {
    const error = fs.readFileSync(
        path.resolve('__tests__/dom/WrapMsoOnDivWithMaxWidthAndFullWidth/error.html'), 'utf8'
    );
    
    const expected = fs.readFileSync(
        path.resolve('__tests__/dom/WrapMsoOnDivWithMaxWidthAndFullWidth/expected.html'), 'utf8'
    );
