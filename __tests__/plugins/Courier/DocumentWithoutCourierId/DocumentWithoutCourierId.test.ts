...
test('inserting the Courier request ID without a head', async() => {
    const error = fs.readFileSync(
        path.resolve('__tests__/plugins/Courier/DocumentWithoutCourierId/error.html'), 'utf8'
    );
    
    const expected = fs.readFileSync(
        path.resolve('__tests__/plugins/Courier/DocumentWithoutCourierId/expected.html'), 'utf8'
    );
