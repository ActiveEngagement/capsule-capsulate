...
test('adding the preview text using plain text to a document with it', async() => {
    const error = fs.readFileSync(
        path.resolve('__tests__/plugins/PreviewText/DocumentWithPreviewTextUsingNull/error.html'), 'utf8'
    );
    
    const expected = fs.readFileSync(
        path.resolve('__tests__/plugins/PreviewText/DocumentWithPreviewTextUsingNull/expected.html'), 'utf8'
    );
