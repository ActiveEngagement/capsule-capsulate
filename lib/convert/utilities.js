...
utilities.getComputedStyle = function(element, ...args) {
    if(!element) {
    ...
    return window.getComputedStyle(element, ...args);
};

utilities.getElementFontSize = function(element) {
    return parseFloat(utilities.getComputedStyle(element, '').fontSize);
};
...
utilities.getRelativeElementDimension = function(element, property) {
    ...
    var positioned = positionTriggers.indexOf(utilities.getComputedStyle(element, '').position) > -1;
    ...
    if(!useSelf && positioned) {
        referenceComputed = utilities.getComputedStyle(reference, '');
